...
click_sound_path = resource_path('media/click.mp3')
font_path = resource_path('fonts/ww2_font.otf')
collect_sound_path = resource_path('media/ammo_collect.mp3')
...
#pozadi
bg = pygame.image.load(resource_path("img/background.jpg")).convert()
...
enemy_counter = 0
enemy_speed = 3
enemy_speed_increase_time = 100000
...
while lets_continue:
    ...
    if pygame.time.get_ticks() - last_increase_time >= enemy_speed_increase_time:
        enemy_speed += 0.5
        last_increase_time = pygame.time.get_ticks()
    ...
    if scroll > 0:
        screen.blit(bg, (0, -bg_height + scroll))
